                    ...
                    <div class="col-md-12">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                @if(!empty($blog->menu))
                                    @php($menu = \App\Modules\Home\Models\Menu::find($blog->menu))
                ...
                <div class="row">
                    <div class="col-md-9">
                        <section class="details">
                            ...
                            <div class="content">
                                <p><strong><i>{{$blog->description}}</i></strong></p>
                                @php
                            ...
                            <div class="meta meta2">
                                ...
                                <ul class="social-share">
                                    <li class="">
                                        <a aria-label="Facebook" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u={{url('/su-kien/'.$blog->slug)}}" class="facebook" rel="nofollow noopener noreferrer">
                                            <i class="fa-brands fa-facebook-f"></i>
                                        </a>
                                    </li>
                                    <li class="">
                                        <a aria-label="Twitter" target="_blank" href="https://twitter.com/intent/tweet?status='{{url('/su-kien/'.$blog->slug)}}" class="twitter" rel="nofollow noopener noreferrer">
                                            <i class="fa-brands fa-twitter"></i>
                                        </a>
                                    </li>
                                    <li class="">
                                        <a aria-label="Pinterest" target="_blank" href="https://pinterest.com/pin/create/button?url={{url('/su-kien/'.$blog->slug)}}" class="pinterest" rel="nofollow noopener noreferrer">
                            ...
                            <div class="comment" id="comment">
                                ...
                                <div class="cmt_list_box">
                                    <ul>
                                        @php($comments = \App\Modules\Comment\Models\Comment::whereNull('parent')->where('status', '=', 1)->where('event_id', '=', $blog->id)->orderBy('created_at', 'ASC')->paginate(10))
                                        ...
                                        @if(!empty($comments))
                                            @foreach($comments as $cmt)
                                                <li>
                                                    ...
                                                    @php($comments_child = \App\Modules\Comment\Models\Comment::where('parent', '=', $cmt->id)->where('status', '=', 1)->where('event_id', '=', $blog->id)->orderBy('created_at', 'ASC')->paginate(10))
                                                    ...
                                                    <form method="POST" action="{{route('comment.event.child')}}" accept-charset="UTF-8" id="cmt_reply" enctype="multipart/form-data">
                                                        ...
                                                        <div class="cmt_form_bottom">
                                                            ...
                                                            <div class="cmt_submit">
                                                                ...
                                                                <input type="hidden" value="{{$blog->id}}" name="product_id" />
                                                                <input type="hidden" value="" name="parent_id" class="parent_id"/>
                                                                <input type="hidden" value="{{url('/su-kien/'.$blog->slug)}}" name="product_slug" />
                                                            </div>
                                                        </div>
                                ...
                                <form method="POST" action="{{route('comment.event')}}" accept-charset="UTF-8" enctype="multipart/form-data">
                                    <input name="_token" type="hidden" value="{{csrf_token()}}">
                                    <div class="cmt_input">
                                ...
                                <div class="row">
                                    @if(!empty($blog_relate))
                                        @foreach($blog_relate as $post)
                                            <div class="col-md-3">
                                                <article>
                                                    <a href="{{url('/su-kien/'.$post->slug)}}" class="img"><img loading="lazy" src="{{Config::get('app.PATH_ADMIN').$post->image}}" alt="{{$post->name}}" /></a>
                        ...
                        <sidebar class="sidebar">
                            <div class="meta">
                                <div class="author">
                                    @if(!empty($blog->author))
                                        <i class="fa fa-user-circle-o" aria-hidden="true"></i> {{\App\Modules\Blog\Models\User::where('user_id', '=', $blog->author)->first()->user_name}}
                                    @endif
                                </div>
                            ...
                            <div class="banner">
                                @if(!empty($banner->image))
                                    <a href="{{$banner->link}}">
                                        <img loading="lazy" src="{{Config::get('app.PATH_ADMIN').$banner->image}}" alt="{{$banner->name}}"/>
                                    </a>
                                @endif
                            </div>
